...
def clean_r_plotly_object(obj, parent_key=None, trace_context=False):
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            ...
            if (
                parent_key in invalid_nested_props
                and key in invalid_nested_props[parent_key]
            ):
                continue
            ...
            cleaned_value = clean_r_plotly_object(
                value, key, trace_context or parent_key == "data"
            )
        ...
        if trace_context and parent_key in trace_array_fields:
            ...
            for item in obj:
                if (
                    ...
                    result.append(
                        clean_r_plotly_object(item, parent_key, trace_context)
                    )
            return result
        ...
        if len(obj) > 1 and all(
            isinstance(item, list) and len(item) == 1 for item in obj
        ):
            return [
...
def get_flow_plot(
    ...
    if return_html:
        ...
        cleaned_data = clean_r_plotly_object(
            plot_data, parent_key=None, trace_context=False
        )
