...
testDir = os.path.dirname(os.path.realpath(__file__))  # located within the src/tests folder
dataDir = os.path.join(testDir, "data")  # data folder within the tests folder
srcDir = os.path.abspath(os.path.join(testDir, ".."))  # go up one level into the src folder
...
def test_get_ref_line_geometry():
    ...
    """
    test_dataset_path = os.path.join(TEST_PARQUET, "blw-elkhart_ref_line_geometry_df.parquet")
...
def test_get_ref_pt_geometry():
    ...
    """
    test_dataset_path = os.path.join(TEST_PARQUET, "blw-elkhart_ref_pt_geometry_df.parquet")
    test_dataset_df = gpd.read_parquet(test_dataset_path)
