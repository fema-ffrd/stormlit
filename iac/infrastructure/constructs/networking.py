        ...
        *,
        ...
        for i, az in enumerate(vpc_subnet_azs):
            ...
            public_subnet = Subnet(
                self, f"public-subnet-{i + 1}", vpc_id=self.vpc.id,
                cidr_block=public_subnet_cidr, availability_zone=az,
                map_public_ip_on_launch=True,
                tags={**tags, "Name": f"{resource_prefix}-public-subnet-{i + 1}", "Type": "Public"},
            )
            self.public_subnets.append(public_subnet)
            RouteTableAssociation(
                self, f"public-rta-{i + 1}", subnet_id=public_subnet.id,
                route_table_id=public_route_table.id,
            )

            private_subnet = Subnet(
                self, f"private-subnet-{i + 1}", vpc_id=self.vpc.id,
                cidr_block=private_subnet_cidr, availability_zone=az,
                tags={**tags, "Name": f"{resource_prefix}-private-subnet-{i + 1}", "Type": "Private"},
            )
            self.private_subnets.append(private_subnet)
            RouteTableAssociation(
                self, f"private-rta-{i + 1}", subnet_id=private_subnet.id,
                route_table_id=private_route_table.id,
            )

            if i == 0: 
                nat_gateway = NatGateway(
                    self, "nat", allocation_id=eip_for_nat.id, subnet_id=public_subnet.id,
                    tags={**tags, "Name": f"{resource_prefix}-nat"},
                )

        if nat_gateway:
            Route(
                self, "private-route-to-nat", route_table_id=private_route_table.id,
                destination_cidr_block="0.0.0.0/0", nat_gateway_id=nat_gateway.id,
            )

        # ALB Security Group
        self.alb_security_group = SecurityGroup(
            self, "alb-sg", name=f"{resource_prefix}-alb-sg", vpc_id=self.vpc.id,
        ...
        for port, desc_suffix in [(80, "HTTP"), (443, "HTTPS")]:
            SecurityGroupRule(
                self, f"alb-ingress-{port}", type="ingress", security_group_id=self.alb_security_group.id,
                from_port=port, to_port=port, protocol="tcp", cidr_blocks=["0.0.0.0/0"],
                description=f"Allow {desc_suffix} from Internet",
            )
        SecurityGroupRule(
            self, "alb-egress", type="egress", security_group_id=self.alb_security_group.id,
        ...
        self.ecs_security_group = SecurityGroup(
            self, "ecs-sg", name=f"{resource_prefix}-ecs-sg", vpc_id=self.vpc.id,
        ...
        SecurityGroupRule(
            self, "ecs-ingress-stormlit-from-alb", type="ingress", security_group_id=self.ecs_security_group.id,
            from_port=8501, to_port=8501, protocol="tcp",
        ...
        SecurityGroupRule(
            self, "ecs-ingress-stac-from-vpc-nlb", type="ingress", security_group_id=self.ecs_security_group.id,
            from_port=stac_api_config.container_port, to_port=stac_api_config.container_port, protocol="tcp",
            cidr_blocks=[vpc_cidr],
            description=f"Allow STAC API traffic from VPC (NLB) on port {stac_api_config.container_port}",
        )
        SecurityGroupRule(
            self, "ecs-self-ingress", type="ingress", security_group_id=self.ecs_security_group.id,
            from_port=0, to_port=0, protocol="-1", self_attribute=True,
            description="Allow ECS instances to communicate with each other",
        )
        SecurityGroupRule(
            self, "ecs-egress", type="egress", security_group_id=self.ecs_security_group.id,
            from_port=0, to_port=0, protocol="-1", cidr_blocks=["0.0.0.0/0"],
            description="Allow all outbound traffic",
        )
        ...
        self.rds_security_group = SecurityGroup(
            self, "rds-sg", name=f"{resource_prefix}-rds-sg", vpc_id=self.vpc.id,
            description=f"Security group for {resource_prefix} RDS instances",
            tags={**tags, "Name": f"{resource_prefix}-rds-sg"},
        )
        SecurityGroupRule(
            self, "rds-vpc-ingress", type="ingress", security_group_id=self.rds_security_group.id,
            from_port=5432, to_port=5432, protocol="tcp",
            source_security_group_id=self.ecs_security_group.id,
            description="Allow PostgreSQL access from ECS tasks",
        )
        SecurityGroupRule(
            self, "rds-egress", type="egress", security_group_id=self.rds_security_group.id,
            from_port=0, to_port=0, protocol="-1", cidr_blocks=["0.0.0.0/0"],
            description="Allow all outbound traffic",
        )
