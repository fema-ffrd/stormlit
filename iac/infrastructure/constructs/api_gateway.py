...
from cdktf_cdktf_provider_aws.apigatewayv2_authorizer import (
...
from cdktf_cdktf_provider_aws.apigatewayv2_domain_name import (
    ...
    def __init__(
        ...
        *,
        ...
        self.stac_nlb_listener = LbListener(
            ...
            default_action=[
                LbListenerDefaultAction(
                    type="forward", target_group_arn=self.stac_nlb_target_group.arn
                )
            ],
            tags=tags,
        )
        ...
        for route_key, route_id_suffix in public_routes_map.items():
            ...
            Apigatewayv2Route(
                self,
                f"stac-public-route-{route_id_suffix}",
                api_id=self.http_api.id,
                route_key=route_key,
        ...
        for method in secured_methods:
            ...
            if route_key_base not in created_routes:
                ...
                Apigatewayv2Route(
                    self,
                    f"stac-secured-route-{method.lower()}-base",
                    api_id=self.http_api.id,
            ...
            if route_key_proxy not in created_routes:
                ...
                Apigatewayv2Route(
                    self,
                    f"stac-secured-route-{method.lower()}-proxy",
                    api_id=self.http_api.id,
        ...
        self.api_gateway_invoke_url_output = TerraformOutput(
            self,
            "stac_api_gateway_invoke_url_regional",
